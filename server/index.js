...
app.get('/items.json', (req, res) => { res.json(itemsJson) })
app.post('/:channelId', (req, res) => {
    const channelId = req.params.channelId;
    const { items } = req.body;
    if (typeof items == 'object') {
        const itemNames = Object.values(items).map(({ name = "empty" }) => name).filter(name => name != "empty");
...
var getUrl = (channelId) => "https://api.twitch.tv/extensions/message/" + channelId
...
var getBody = itemsList => ({
    "content_type": "application/json", "message": JSON.stringify(
        {
            "items": itemsList
        }
    ), "targets": ["broadcast"]
